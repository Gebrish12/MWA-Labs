...
const getAll = function(req,res){
    ...
    Company.findById(companyID).select("carModels").exec(function(err,cars){
        if(err){
...
const deletecar = function(req,res){
 ...
 const updatecar = function(req,res){
    ...
    const carID = req.params.carID;
    if (!mongoose.Types.ObjectId.isValid(companyID)){
             res.status(400).json("the id that you provide doesn't satisfy the company id");
             return;
    }
    if (!mongoose.Types.ObjectId.isValid(carID)){
             res.status(400).json("the id that you provide doesn't satisfy the company id");
                 ...
                 const car =company.carModels.id(carID);
                    if (car){
                        ...
                        company.save(function(err,result){
                          ...
                          res.status(404).json("the company with the given id is not available")
                      }
